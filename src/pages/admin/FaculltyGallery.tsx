...
//       if (id) {
//         setIsEditMode(true);
//         const faculty = faculties.find((f: { id: string; }) => f.id === id);
//         if (faculty) {
//           setFormData({
//             name: faculty.name || "",
//             designation: faculty.designation || "",
...
//       toast.info("File size exceeds 2 MB!");
...
//        toast.info("Image resolution must be 400x500 px!");
//         setIsValidImage(false);
//         return;
//       }
//       // Valid image
...
// //     ✅ <strong>Recommended size:</strong> 400 × 500 px (4:5 ratio)
// // <br/>

// //     ⚠️ <strong>Max file size:</strong> 2 MB
// //   </p>
// //         </div>
...
// //   className="w-full bg-blue-600 text-white px-5 py-3 rounded-lg font-semibold hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
// // >
// //   {isEditMode ? "Update" : "Create"}
// // </button>
...
export default function FacultyGallery() {
    ...
    const loadFaculty = async () => {
      ...
      if (id) {
        ...
        if (faculty) {
          ...
          setPreview(faculty.imageUrl || null);
        }
      }
    };
    loadFaculty();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode && id) {
        await updateFaculty(id, formData as any); // cast to satisfy Pocketbase function typing
        toast.success("Faculty updated successfully!");
      } else {
        await createFaculty(formData as any);
        toast.success("Faculty added successfully!");
        setFormData({
  name: "",
  designation: "",
