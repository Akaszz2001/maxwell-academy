...
//   const [preview, setPreview] = useState<string | null>(null);
//   const [loading, setLoading] = useState(false);
//   const [isValidImage, setIsValidImage] = useState(true); // ✅ Track image validity
...
//         toast.info("File size exceeds 2MB limit!");
//         setIsValidImage(false);
//         return;
//       }

//       // ✅ Check image resolution (must be 300x400)
//       const img = new Image();
//       img.onload = () => {
//         if (img.width !== 300 || img.height !== 400) {
//           toast.info("Image resolution must be exactly 300×400 pixels!");
//           setIsValidImage(false);
//         } else {
...
//     e.preventDefault();
//     if (!isValidImage) {
//       toast.info("Please upload a valid image before submitting!");
//       return;
//     }
...
//             ✅ <strong>Recommended:</strong> 300×400 px (passport size) <br />
//             ⚠️ <strong>Max file size:</strong> 2MB
//           </p>
...
export default function PerformerForm() {
  ...
  const [formData, setFormData] = useState<Partial<Performer>>({
    name: "",
    rank: 0,
    photo: undefined, // ✅ File for upload or string URL
  });
    ...
    if (id && performers.length > 0) {
      ...
      if (performer) {
        setFormData({
          name: performer.name,
          rank: performer.rank,
          photo: performer.photo, // string URL
        });
        setPreview(typeof performer.photo === "string" ? performer.photo : null);
      }
    }
  }, [id, performers]);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files[0]) {
      ...
      img.src = URL.createObjectURL(file);
const payload=((prev: any) => ({ ...prev, photo: file }))
      setFormData(payload);
      setPreview(URL.createObjectURL(file));
    } else {
      setFormData(prev => ({ ...prev, [name]: value }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (id) {
        await updatePerformer(id, formData as Performer);
        toast.success("Performer updated successfully!");
      } else {
        await createPerformer(formData as Performer);
        toast.success("Performer added successfully!");
      }
